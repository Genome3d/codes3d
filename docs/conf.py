...
SNP_DATABASE_FP = LIB_DIR + "snp_index_dbSNP_b147.db" #The database of SNPs to search for details on input dbSNP IDs
...
GENE_BED_FP = LIB_DIR + "gene_index_GTEx.bed" #Bed file detailing locations of genes in genome (the UCSC known gene list for hg19 by default).
GENE_DATABASE_FP = LIB_DIR + "gene_index_GTEx.db" #Database constructed from the gene BED file.
...
EXPRESSION_TABLE_FP = LIB_DIR + "GTEx_Analysis_v6p_RNA-seq_gene_median_rpkm.gct" #The tab-separated table containing expression information about GTEx genes (the GTEx Analysis V6 RNA-seq gene median by default).
