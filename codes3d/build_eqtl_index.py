...
def build_eqtl_table_index(table_fp,snp_col,gene_symbol_col,gene_chr_col,gene_start_col,gene_stop_col,output_fp):
	...
	table_index.execute("CREATE TABLE IF NOT EXISTS eqtls (rsID text, gene_name text, gene_chr text, gene_start integer, gene_end integer)")
	...
	for i,line in enumerate(eqtl_table):
		...
		table_index.execute("INSERT INTO eqtls VALUES (?,?,?,?,?)",[eqtl[snp_col],eqtl[gene_symbol_col],eqtl[gene_chr_col],eqtl[gene_start_col],eqtl[gene_stop_col]])
	eqtl_table.close()
...
if __name__ == "__main__":
	...
	build_eqtl_table_index(args.input_eqtl_table_fp,args.snp_id_column,args.gene_symbol_column,args.gene_chr_column,args.gene_start_column,args.gene_stop_column,args.output_fp)
