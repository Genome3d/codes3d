...
def _check_dependency(name):
    e = subprocess.call('which '+name, shell=True,
...
def _impute_mean(g, verbose=False):
    ...
    if verbose and n > 0:
        print(
            '    imputed at least 1 sample in {}/{} sites'.format(n, g.shape[0]))
...
class PlinkReader(object):
    def __init__(self, plink_prefix_path, select_samples=None, exclude_variants=None, exclude_chrs=None, verbose=True, dtype=np.int8):
        ...
        """
        self.bim, self.fam, self.bed = read_plink(
            plink_prefix_path, verbose=verbose)
        self.bed = 2 - self.bed  # flip allele order: PLINK uses REF as effect allele
        if dtype == np.int8:
            # convert missing (NaN) to -1 for int8
        ...
        self.variant_pos = {i: g['pos'] for i, g in self.bim.set_index(
            'snp')[['chrom', 'pos']].groupby('chrom')}
    ...
    def get_genotype(self, variant_id, sample_ids=None, impute=False, verbose=False, dtype=np.int8):
        ...
        g, _ = self.get_genotypes(
            [variant_id], sample_ids=sample_ids, impute=impute, verbose=verbose, dtype=dtype)
...
def load_genotypes(plink_prefix_path, select_samples=None, dtype=np.int8):
    pr = PlinkReader(plink_prefix_path,
                     select_samples=select_samples, dtype=dtype)
    print('Loading genotypes ... ', end='', flush=True)
    df = pd.DataFrame(pr.get_all_genotypes(),
                      index=pr.bim['snp'], columns=pr.fam['iid'])
...
def get_vcf_region(region_str, vcfpath, field='GT', sample_ids=None, select_samples=None, impute_missing=True):
    ...
    s = subprocess.check_output(
        'tabix {} {}'.format(vcfpath, region_str), shell=True)
    ...
    g = np.array([parse_genotypes([i.split(':')[ix]
                                   for i in line[9:]], field=field) for line in s])
...
def get_vcf_variants(variant_ids, vcfpath, field='GT', sample_ids=None):
    ...
    with tempfile.NamedTemporaryFile() as regions_file:
        df = pd.DataFrame([i.split('_')[:2]
                           for i in variant_id_set], columns=['chr', 'pos'])
        df['pos'] = df['pos'].astype(int)
        df = df.sort_values(['chr', 'pos'])
        df.to_csv(regions_file.name, sep='\t', index=False, header=False)
        s = subprocess.check_output(
            'tabix {} --regions {}'.format(vcfpath, regions_file.name), shell=True)
    s = s.decode().strip().split('\n')
    s = [i.split('\t') for i in s]
    ...
    g = np.array([i for i in g if -1 not in i])
...
class GenotypeGeneratorTrans(object):
    def __init__(self, genotype_df, batch_size=50000, chr_s=None):
        ...
        if chr_s is not None:
            ...
            for k, c in enumerate(self.chroms):
                ...
                batch_indexes = [
                    [chr_ix[k]+i*batch_size, chr_ix[k]+(i+1)*batch_size] for i in range(num_batches)]
    ...
    def generate_data(self, chrom=None, verbose=False, enum_start=1):
        ...
        if chrom is None:
            ...
            num_batches = np.sum([len(i)
                                  for i in self.chr_batch_indexes.values()])

        for k, i in enumerate(batch_indexes, enum_start):  # loop through batches
            if verbose:
                print_progress(k, num_batches, 'batch')
            g = self.genotype_df.values[i[0]:i[1]].astype(np.int8)
...
class InputGeneratorCis(object):
    ...
    def __init__(self, genotype_df, variant_df, phenotype_df, phenotype_pos_df, group_s=None, window=1000000):
        ...
        for k, phenotype_id in enumerate(phenotype_df.index, 1):
            if np.mod(k, 1000) == 0:
                print('\r  * checking phenotypes: {}/{}'.format(k,
                                                                phenotype_df.shape[0]), end='')
            ...
            lb = np.searchsorted(
                self.chr_variant_dfs[chrom]['pos'].values, tss - self.window)
            ub = np.searchsorted(
                self.chr_variant_dfs[chrom]['pos'].values, tss + self.window, side='right')
            if lb != ub:
    ...
    def generate_data(self, chrom=None, verbose=False):
        ...
        if chrom is None:
            ...
            if self.group_s is None:
                offset_dict = {i: j for i, j in zip(
                    *np.unique(self.phenotype_pos_df['chr'], return_index=True))}
            else:
                offset_dict = {i: j for i, j in zip(
                    *np.unique(self.phenotype_pos_df['chr'][self.group_s.drop_duplicates().index], return_index=True))}
        ...
        if self.group_s is None:
            ...
            for k, (group_id, g) in enumerate(gdf, chr_offset+1):
                ...
                assert np.all([self.cis_ranges[g.index[0]][0] == self.cis_ranges[i][0]
                               and self.cis_ranges[g.index[0]][1] == self.cis_ranges[i][1] for i in g.index[1:]])
                group_phenotype_ids = g.index.tolist()
                # p = self.phenotype_df.loc[group_phenotype_ids].values
                p = self.phenotype_df.values[[index_dict[i]
                                              for i in group_phenotype_ids]]
